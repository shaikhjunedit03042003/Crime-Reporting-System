...
public class PoliceOfficerService {
	...
	public PoliceOfficerBO updatePoliceOfficer(Long policeId, PoliceOfficerBO policeOfficerBO) {
		...
		if (entity != null) {
			PoliceOfficerEntity entityForSave=PoliceOfficerEntity.of()
					.badgeNumber(policeOfficerBO.getBadgeNumber())
					.contactNumber(policeOfficerBO.getContactNumber())
					.contentType(policeOfficerBO.getContentType())
					.email(policeOfficerBO.getEmail())
					.filename(policeOfficerBO.getFilename())
					.id(policeOfficerBO.getId())
					.imgaes(policeOfficerBO.getImages())
					.name(policeOfficerBO.getName())
					.policeStation(policeOfficerBO.getPoliceStation())
					.policeStationselect(policeOfficerBO.getPoliceStationselect())
					.rank(policeOfficerBO.getRank())
					.build();
			 PoliceOfficerEntity	policeEntity2=	policeOfficerRepository.save(entityForSave);
					return PoliceOfficerBO.of()
							.badgeNumber(policeEntity2.getBadgeNumber())
							.contactNumber(policeEntity2.getContactNumber())
							.contentType(policeEntity2.getContentType())
							.email(policeEntity2.getEmail())
							.filename(policeEntity2.getFilename())
							.id(policeEntity2.getId())
							.images(policeEntity2.getImgaes())
							.name(policeEntity2.getName())
							.policeStation(policeEntity2.getPoliceStation())
							.policeStationselect(policeEntity2.getPoliceStationselect())
							.rank(policeEntity2.getRank())
